...
**根因**：在线笔记本分区未在本地缓存，图片数据未下载。

**解决（有效路径，按优先级）**：
1. **OneNote UI**：文件 → 选项 → 同步 → 同步所有文件和图像，**勾选"下载全部文件和图像"** → 确定；再手动打开目标分区/页面（触发下载），等图片正常显示、同步图标消失
2. **COM 直读**（已下载后）：按 Section ID `GetHierarchy` + `GetPageContent(..., 1)` 直接读取在线分区，**无需导出 .one**（实测：用户勾选设置+手动打开后，COM 直读全部拿到 Data）
3. 导出 .one 兜底：文件 → 导出 → 分区 → OneNote 格式

**无效方式（实测）**：`SyncHierarchy(分区ID)`（单参版本调用成功但 3 分钟无下载）、`NavigateTo(分区ID)`（UI 跳转但 3 分钟无下载）——COM API 无法强制下载云端内容。

**注意**：导出/读取只包含**已加载过**的页面内容——没打开过的页面图片仍会缺（实测 TI开发.one 137MB：打开过的页面有图，未打开页面 Image=0）。打开页面后如果图片本身是坏链（其他设备编辑未完整同步，页面里 1 好 1 坏两张图），则该图无法找回——源数据损坏，非流程问题。
...
- **在线笔记本分区**：先按上文"在线分区图片未下载"处理（勾选下载全部文件和图像 → 打开页面 → COM 直读或导出 .one），否则读到的文件缺图
...
5. 不修改 .one 原文件（只读打开）
6. **md 引用配对必须数量一致**：md 图片引用数 ≠ XML `<one:Image>` 数时**禁止强行配对**（8-12 实测降级配对把 I2S（一）4 处断链错配成错误图）——只按"数量一致、顺序对齐"配对；引用混排 wikilink（成功）与 markdown（断链，URL 编码），非图片附件（.cpp/.h/.docx/.csv/.xlsx）必须过滤
7. **`<one:Data>` 可能是嵌入对象而非图片**：base64 解出后魔数可能是 `010000006c`（OneNote 对象头）——写文件前校验 PNG/JPEG/GIF 魔数，无效则不写并标记
8. 源数据损坏的辨识：同一位置 2 张图（1 好 1 坏）、页面 lastModifiedTime 久远但图片缺失、全部断链扩展名相同（如全 .jpeg）、页面图数远少于 md 引用数
